...
#######----
# Efficient Generalized prediction function
cos_pred_fun_efficient <- function(sig_names, sample_names, mut, emb_len = 200, emb = embeddings, 
  ...
  preds_embeddings <- mut[,colnames(mut) %in% c("sample", "Tumor_Sample_Barcode", "mut_1536")] %>%
    left_join(embeddings %>% rownames_to_column("mut_1536"), 
              by = "mut_1536")
